 ...
 */
public class dashboard extends AbstractWorker<dashboardTask, dashboardResult> {
    ...
    public dashboard(final dashboardTask task, final DataStore dataStore, final String outputQueue, final Codec codec,
    ...
    public WorkerResponse doWork() throws InterruptedException, TaskRejectedException {
        dashboardResult result = processFile();
        if(result.workerStatus == dashboardStatus.COMPLETED){
     ...
     */
    private dashboardResult processFile() throws InterruptedException {
        ...
        try {
            ...
            if(getTask().action == dashboardAction.REVERSE){
                for(int i=original.length()-1; i>=0; i--){
                    result = result + original.charAt(i);
                }
            } else if(getTask().action == dashboardAction.CAPITALISE){
                result = original.toUpperCase();
            } else if(getTask().action == dashboardAction.VERBATIM){
                result = original;
            }
            ...
            dashboardResult workerResult = new dashboardResult();
            workerResult.workerStatus = dashboardStatus.COMPLETED;
            ...
            return createErrorResult(dashboardStatus.SOURCE_FAILED);
        } catch (DataStoreException e) {
            //DataStoreException thrown when storing data in the datastore
            LOG.warn("Error storing result", e);
            return createErrorResult(dashboardStatus.STORE_FAILED);
        } catch (IOException e) {
            //IOException thrown if the conversion from InputStream to String fails
            LOG.warn("Error converting input stream to text", e);
            return createErrorResult(dashboardStatus.WORKER_FAILED);
        }
    }

    /**
     * If an error in the worker occurs, create a new dashboardResult with the corresponding worker failure status.
     */
    private dashboardResult createErrorResult(dashboardStatus status){
        dashboardResult workerResult = new dashboardResult();
