 ...
 */
public class rtfs extends AbstractWorker<rtfsTask, rtfsResult> {

    /**
     * Logger for logging purposes.
     */
    private static final Logger LOG = LoggerFactory.getLogger(rtfs.class);
    ...
    public rtfs(final rtfsTask task, final DataStore dataStore, final String outputQueue, final Codec codec,
    ...
    public String getWorkerIdentifier() {
        return rtfsConstants.WORKER_NAME;
    }

    @Override
    public int getWorkerApiVersion() {
        return rtfsConstants.WORKER_API_VER;
    }
    ...
    public WorkerResponse doWork() throws InterruptedException, TaskRejectedException {
        rtfsResult result = processFile();
        if(result.workerStatus == rtfsStatus.COMPLETED){
     ...
     */
    private rtfsResult processFile() throws InterruptedException {
        ...
        try {
            ...
            if(getTask().action == rtfsAction.REVERSE){
                for(int i=original.length()-1; i>=0; i--){
                    result = result + original.charAt(i);
                }
            } else if(getTask().action == rtfsAction.CAPITALISE){
                result = original.toUpperCase();
            } else if(getTask().action == rtfsAction.VERBATIM){
                result = original;
            }
            ...
            rtfsResult workerResult = new rtfsResult();
            workerResult.workerStatus = rtfsStatus.COMPLETED;
            ...
            return createErrorResult(rtfsStatus.SOURCE_FAILED);
        } catch (DataStoreException e) {
            //DataStoreException thrown when storing data in the datastore
            LOG.warn("Error storing result", e);
            return createErrorResult(rtfsStatus.STORE_FAILED);
        } catch (IOException e) {
            //IOException thrown if the conversion from InputStream to String fails
            LOG.warn("Error converting input stream to text", e);
            return createErrorResult(rtfsStatus.WORKER_FAILED);
        }
    }

    /**
     * If an error in the worker occurs, create a new rtfsResult with the corresponding worker failure status.
     */
    private rtfsResult createErrorResult(rtfsStatus status){
        rtfsResult workerResult = new rtfsResult();
