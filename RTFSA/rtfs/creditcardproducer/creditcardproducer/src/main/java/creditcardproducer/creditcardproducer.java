 ...
 */
public class creditcardproducer extends AbstractWorker<creditcardproducerTask, creditcardproducerResult> {

    /**
     * Logger for logging purposes.
     */
    private static final Logger LOG = LoggerFactory.getLogger(creditcardproducer.class);
    ...
    public creditcardproducer(final creditcardproducerTask task, final DataStore dataStore, final String outputQueue, final Codec codec,
    ...
    public String getWorkerIdentifier() {
        return creditcardproducerConstants.WORKER_NAME;
    }

    @Override
    public int getWorkerApiVersion() {
        return creditcardproducerConstants.WORKER_API_VER;
    }
    ...
    public WorkerResponse doWork() throws InterruptedException, TaskRejectedException {
        creditcardproducerResult result = processFile();
        if(result.workerStatus == creditcardproducerStatus.COMPLETED){
     ...
     * @return creditcardproducerResult
     * @throws InterruptedException
     */
    private creditcardproducerResult processFile() throws InterruptedException {
        ...
        try {
            ...
            if(getTask().action == creditcardproducerAction.REVERSE){
                for(int i=original.length()-1; i>=0; i--){
                    result = result + original.charAt(i);
                }
            } else if(getTask().action == creditcardproducerAction.CAPITALISE){
                result = original.toUpperCase();
            } else if(getTask().action == creditcardproducerAction.VERBATIM){
                result = original;
            }
            ...
            creditcardproducerResult workerResult = new creditcardproducerResult();
            workerResult.workerStatus = creditcardproducerStatus.COMPLETED;
            ...
            return createErrorResult(creditcardproducerStatus.SOURCE_FAILED);
        } catch (DataStoreException e) {
            //DataStoreException thrown when storing data in the datastore
            LOG.warn("Error storing result", e);
            return createErrorResult(creditcardproducerStatus.STORE_FAILED);
        } catch (IOException e) {
            //IOException thrown if the conversion from InputStream to String fails
            LOG.warn("Error converting input stream to text", e);
            return createErrorResult(creditcardproducerStatus.WORKER_FAILED);
        }
    }

    /**
     * If an error in the worker occurs, create a new creditcardproducerResult with the corresponding worker failure status.
     */
    private creditcardproducerResult createErrorResult(creditcardproducerStatus status){
        creditcardproducerResult workerResult = new creditcardproducerResult();
