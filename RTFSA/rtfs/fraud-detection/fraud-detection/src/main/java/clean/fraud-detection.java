 ...
 */
public class fraud-detection extends AbstractWorker<fraud-detectionTask, fraud-detectionResult> {

    /**
     * Logger for logging purposes.
     */
    private static final Logger LOG = LoggerFactory.getLogger(fraud-detection.class);
    ...
    public fraud-detection(final fraud-detectionTask task, final DataStore dataStore, final String outputQueue, final Codec codec,
    ...
    public WorkerResponse doWork() throws InterruptedException, TaskRejectedException {
        fraud-detectionResult result = processFile();
        if(result.workerStatus == fraud-detectionStatus.COMPLETED){
     ...
     * @return fraud-detectionResult
     * @throws InterruptedException
     */
    private fraud-detectionResult processFile() throws InterruptedException {
        ...
        try {
            ...
            if(getTask().action == fraud-detectionAction.REVERSE){
                for(int i=original.length()-1; i>=0; i--){
                    result = result + original.charAt(i);
                }
            } else if(getTask().action == fraud-detectionAction.CAPITALISE){
                result = original.toUpperCase();
            } else if(getTask().action == fraud-detectionAction.VERBATIM){
                result = original;
            }
            ...
            fraud-detectionResult workerResult = new fraud-detectionResult();
            workerResult.workerStatus = fraud-detectionStatus.COMPLETED;
            ...
            return createErrorResult(fraud-detectionStatus.SOURCE_FAILED);
        } catch (DataStoreException e) {
            //DataStoreException thrown when storing data in the datastore
            LOG.warn("Error storing result", e);
            return createErrorResult(fraud-detectionStatus.STORE_FAILED);
        } catch (IOException e) {
            //IOException thrown if the conversion from InputStream to String fails
            LOG.warn("Error converting input stream to text", e);
            return createErrorResult(fraud-detectionStatus.WORKER_FAILED);
        }
    }

    /**
     * If an error in the worker occurs, create a new fraud-detectionResult with the corresponding worker failure status.
     */
    private fraud-detectionResult createErrorResult(fraud-detectionStatus status){
        fraud-detectionResult workerResult = new fraud-detectionResult();
